...
export default function ReportsDashboard() {
  ...
  if (!appointments || !services || !users) return <div>Loading...</div>;
  ...
  const popularServices = Object.entries(serviceCount)
    .map(([id, count]) => ({ name: servicesById[id]?.name || "Unknown", count }))
  ...
  const topCustomers = Object.entries(userCount)
    .map(([id, count]) => ({
      name: `${usersById[id]?.firstName || "Unknown"} ${usersById[id]?.lastName || ""}`.trim(),
      count
    }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 5);
